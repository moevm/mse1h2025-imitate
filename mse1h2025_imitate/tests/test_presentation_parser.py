...
from ..graduation_imitator.dto.PresentationData import PresentationData
from ..graduation_imitator.services.PresentationParserService import PresentationParserService


def test_parser_must_raise_error_if_invalid_file_format():
    invalid_file_format_path = os.path.join(BASE_DIR, 'tests', 'resources', 'test_presentations', 'invalid_format.ppt')
    with pytest.raises(PackageNotFoundError):
        PresentationParserService.parsePPTX(invalid_file_format_path)


def test_parser_must_raise_error_if_file_does_not_exist():
    non_existing_file_path = os.path.join(BASE_DIR, 'tests', 'resources', 'test_presentations', 'non_existing_file.pptx')
    with pytest.raises(PackageNotFoundError):
        PresentationParserService.parsePPTX(non_existing_file_path)
...
def test_parser_must_return_correct_data_if_all_needed_info_exists():
    ...
    parsed_data = PresentationParserService.parsePPTX(
        os.path.join(BASE_DIR, 'tests', 'resources', 'test_presentations', 'template.pptx'))
    assert data_ == parsed_data


def test_parser_must_set_topic_to_not_found_if_topic_not_found():
    presentation_data = PresentationParserService.parsePPTX(
        os.path.join(BASE_DIR, 'tests', 'resources', 'test_presentations', 'without_topic.pptx'))
...
def test_parser_must_set_author_to_not_found_if_author_not_found():
    presentation_data = PresentationParserService.parsePPTX(
        os.path.join(BASE_DIR, 'tests', 'resources', 'test_presentations', 'without_author.pptx'))
    assert presentation_data.topic is not None
    assert presentation_data.topic != 'Not found'
    assert presentation_data.goalAndTasks is not None
    assert presentation_data.goalAndTasks != 'Not found'
...
def test_parser_must_set_goal_and_tasks_to_not_found_if_goal_and_tasks_not_found():
    presentation_data = PresentationParserService.parsePPTX(
        os.path.join(BASE_DIR, 'tests', 'resources', 'test_presentations', 'without_goal_and_tasks.pptx'))
    assert presentation_data.topic is not None
    assert presentation_data.topic != 'Not found'
    assert presentation_data.goalAndTasks is not None
    assert presentation_data.goalAndTasks == 'Not found'
...
def test_parser_must_set_values_to_not_found_if_presentation_is_empty():
    presentation_data = PresentationParserService.parsePPTX(
        os.path.join(BASE_DIR, 'tests', 'resources', 'test_presentations', 'empty.pptx'))
    assert presentation_data.topic is not None
    assert presentation_data.topic == 'Not found'
    assert presentation_data.goalAndTasks is not None
    assert presentation_data.goalAndTasks == 'Not found'

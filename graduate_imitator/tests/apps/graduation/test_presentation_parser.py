...
def test_parser_must_return_correct_data_if_all_needed_info_exists():
    data_ = PresentationData(
        ...
        goalAndTasks=('Цель и задачи Актуальность: ручное распознавание  занимает '
        ...
        author='иванов иван иванович, гр. 5381',
        slidesTitles=['Тема дипломной работы', 'Цель и задачи', 'Подробнее о постановке задачи', 'Задача 1',
         'Задача 2', 'Задача 3', 'Задача 4', 'Заключение', 'Апробация работы', 'Запасные слайды',
         'Практическая значимость']
    )
    parsed_data = PresentationParser.parsePPTX(os.path.join(BASE_DIR, 'tests', 'resources', 'test_presentations', 'template.pptx'))
    assert data_ == parsed_data

def test_parser_must_set_topic_to_not_found_if_topic_not_found():
    presentation_data = PresentationParser.parsePPTX(os.path.join(BASE_DIR, 'tests', 'resources', 'test_presentations', 'without_topic.pptx'))
...
def test_parser_must_set_author_to_not_found_if_author_not_found():
    presentation_data = PresentationParser.parsePPTX(os.path.join(BASE_DIR, 'tests', 'resources', 'test_presentations', 'without_author.pptx'))
    assert presentation_data.topic is not None
    assert presentation_data.topic != 'Not found'
    assert presentation_data.goalAndTasks is not None
    assert presentation_data.goalAndTasks != 'Not found'
...
def test_parser_must_set_goal_and_tasks_to_not_found_if_goal_and_tasks_not_found():
    presentation_data = PresentationParser.parsePPTX(os.path.join(BASE_DIR, 'tests', 'resources', 'test_presentations', 'without_goal_and_tasks.pptx'))
    assert presentation_data.topic is not None
    assert presentation_data.topic != 'Not found'
    assert presentation_data.goalAndTasks is not None
    assert presentation_data.goalAndTasks == 'Not found'
...
def test_parser_must_set_values_to_not_found_if_presentation_is_empty():
    presentation_data = PresentationParser.parsePPTX(os.path.join(BASE_DIR, 'tests', 'resources', 'test_presentations', 'empty.pptx'))
    assert presentation_data.topic is not None
    assert presentation_data.topic == 'Not found'
    assert presentation_data.goalAndTasks is not None
    assert presentation_data.goalAndTasks == 'Not found'
